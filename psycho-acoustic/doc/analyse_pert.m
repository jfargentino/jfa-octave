function analyse_pert( directory, nb, wid, bw, tnmr, len, nf )
%
%function analyse_pert( directory [, nb, wid, bw, tnmr, len, nf ] )
%
...
%des sons ainsi formates, on applique MEAN_PERTINENTS.
%   Les arguments 'nb', 'bw', 'tnmr' ont le meme role que
%pour MEAN_PERTINENTS. Enfin les arguments 'len' et 'nf'
...
%
%Voir aussi PREPARE_DIR, MEAN_PERTINENTS, ANALYSE_HARM,
%           ANALYSE_PERT_HARM.
%
switch( nargin )
...
%recuperations de tous les sous repertoires
subdir = get_all_dir('.');
N      = length(subdir);
...
tic
%traitement pour chacun des sous repertoires
for( n=1:N )
   cd(subdir{n});
   mean_pertinents(get_all_files(subdir{n}),nb,wid,bw,tnmr);
   cd('..');
end%for
t=toc;
