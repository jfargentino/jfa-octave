function psd = get_psd(sig,fs,fig)
%
%function get_psd( sig )
%function get_psd( sig, fs )
%function get_psd( sig, fs, fig )
%function get_psd( wavfile, [ fig ] )
%function psd = get_psd(...)
%
%   Affiche le spectre de puissance de 'sig' selon la frequence 
%d'echantillonnage 'fs' et le retourne sous la forme du vecteur 
%'psd' si ce dernier est precise, contenant egalement les 
%frequences negatives, ce qui est utile pour GET_ALL_TONES par
%exemple. 'fs' n'est necessaire que si on veut l'abscisse en hertz.
%Enfin on peut indiquer le numero de figure grace a 'fig'. A noter
%qu'aucun fenetrage n'est effectue, donc c'est a la charge de 
...
%principal ). Demander a Florent Jaillet pour plus de details.
%
%Voir aussi GET_LPSD.
%
