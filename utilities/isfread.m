...
if isempty(npts)
    ...
    vscale = getnum(hdata, 'VSCALE');
    hscale = getnum(hdata, 'HSCALE');
    vpos = getnum(hdata, 'VPOS');
    voffset = getnum(hdata, 'VOFFSET');
    hdelay = getnum(hdata, 'HDELAY');
    curvid = getstr(hdata, 'CURV');
end;

if ((bytenum ~= 2) | ...
	(bitnum ~= 16) | ...
	not(strcmp(encoding(1:3),'BIN')) | ...
